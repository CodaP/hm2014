...
public  class diceCalculator {
	...
	public static dieGroup calculateDice(int damage, double variance){
		...
		int secondaryDieNum=getDieNum((int)(diceDamage-totalDamage),temp); //+.2
		...
		if(!earlyExit){
			...
			if(secondaryDieNum==0){ //bounds check assign vars
				secondaryDieNum=getDieNum((int)(diceDamage-totalDamage),temp);
				secondaryDieType=diceType;
				totalDamage+=(secondaryDieNum*(1+diceType)/2.0);
			}
